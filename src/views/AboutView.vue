...
//     const imgUrlRes = await FileControllerService.uploadFileUsingPost(res.data);
...
/* 现有样式保留，添加以下新样式 */
.arco-upload-list-picture.custom-upload-avatar:hover .arco-upload-list-picture-mask {
  display: flex; /* 显示编辑图标时使用flex布局居中 */
}

.arco-upload-list-picture.custom-upload-avatar {
  cursor: pointer; /* 改进用户体验，让用户知道这是可点击的 */
  border-radius: 50%; /* 使头像显示为圆形 */
  overflow: hidden; /* 确保头像圆形裁剪效果 */
}
